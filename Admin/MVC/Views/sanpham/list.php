  ...
  <tbody>
    ...
    foreach ($data as $row) { $i=$i+1; ?>
      <tr>
        ...
        <td><?= $row['TenSP'] ?></td>
        <td>
          <img src="../public/<?= $row['HinhAnh1'] ?>" height="60px" >
        </td>
...
?>
        ...
        <td>
         ...
         <?php if (isset($_SESSION['isLogin_Admin']) && $_SESSION['isLogin_Admin'] == true ||isset($_SESSION['isLogin_AdminTong']) && $_SESSION['isLogin_AdminTong'] == true) { ?>
          <a href="?mod=sanpham&act=edit&id=<?= $row['MaSP'] ?>" style = "margin: 3px" type="button" class="btn btn-warning">Sửa</a>
          <a href="?mod=sanpham&act=delete&id=<?= $row['MaSP'] ?>" onclick="return confirm('Bạn có thật sự muốn xóa ?');" style = "margin: 3px" type="button" class="btn btn-danger">Xóa</a>
          <?php } ?>
        </td>
      </tr>
    <?php } ?>
